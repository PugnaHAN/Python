...
('37634.17', 35)
>>> for bike in bicycles:
...     print("{0:<10}{1:<30}".format(bike.identity, bike.name))
>>> bicycles["CLAMEL"].name
'Classic Melbourne'
...
import struct
from BinaryRecordFile_ans import BinaryRecordFile
...
class BikeStock:
    ...
    def __init__(self, filename):
        self.__file = BinaryRecordFile(filename,
                                      _BIKE_STRUCT.size)
    ...
    @property
    def index_from_identity(self):
        return self.__index_from_identity
    ...
    def __delitem__(self, identity):
        ...
        for data in self.__index_from_identity:
            if self.__index_from_identity[data] > index:
                self.__index_from_identity[data] -= 1
