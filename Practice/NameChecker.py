...
def compare(name_descriptor, pcl_holder):
    if isinstance(name_descriptor, NameDescriptor) and isinstance(pcl_holder, PclAnalyst):
        result = [("paper_size", name_descriptor.paper_size == pcl_holder.paper_size),
                  ("paper_type", name_descriptor.paper_type == pcl_holder.paper_type),
                  ("print_quality", name_descriptor.print_quality == pcl_holder.print_quality),
                  ("page_number", name_descriptor.page_number == pcl_holder.page_number)]
        return result
    else:
...
def gen_log(filename, name_descriptor, pcl_holder):
    if isinstance(name_descriptor, NameDescriptor) and isinstance(pcl_holder, PclAnalyst):
        ...
        for result in compare(name_descriptor, pcl_holder):
            if not result[1]:
                name = vars(name_descriptor)["__" + result[0] + "__"]
                pcl = pcl_holder.__property_map__[result[0]]
                msg += "{0:30}{1} -> name - {2}, pcl - {3}\n".format(" ", result[0], name, pcl)
        if msg == os.path.basename(filename) + ":\n":
            msg = "{0}: matched!\n".format(os.path.basename(filename))
        return msg
    else:
        raise TypeError("Wrong type of arguments")
