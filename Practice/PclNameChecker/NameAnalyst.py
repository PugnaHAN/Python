...
class NameDescriptor:
    def  __init__(self, src_name = 'Unknown',
        ...
        self.__print_quality__ = print_quality
    ...
    def analysis_from_filename(self, filename):
        if isinstance(filename, str):
            ...
            self.paper_size = PaperSize.decode_from_str(str(file_descriptor[1]).
                                                        replace("_", " ").strip())
            self.paper_type = PaperType.decode_from_str(file_descriptor[2])
            self.print_quality = PrintQuality.decode_from_str(file_descriptor[3])
    ...
    def _smart_analysis(self, filename):
        ...
        paper_size = "Unknown"
        supported_paper_size = ['_A4_', '_A5_', '_A_', '_B5_',
                                '_4x6_', '_5x7_', '_3x5_',
                                '_Index_', 'Hagiki', '_Legal_']
        ...
        if idx_of_type != 0:
            ...
            if len(paper_type) == 1 and paper_type != 'P':
                paper_type = 'Unknown'
                print_quality = name_lst[idx_of_type]
            else:
                print_quality = name_lst[idx_of_type + 1] if idx_of_type < len(name_lst) - 1 else 'Unknown'
        else:
            for idx in range(len(name_lst)):
                print_quality = PrintQuality.decode_from_str(name_lst[idx])
                if print_quality != PrintQuality.Unknown:
                    print_quality = print_quality.name
        ...
        return [file_name, (paper_size.replace("_", " ") if paper_size is not None else 'Unknown'),
                paper_type, print_quality, full_bleed, page_number]
...
if __name__ == '__main__':
    filename = 'Taiji__official__PrintFiles__ComPhoto_B5_P_N_2pgs__current.pcl'
